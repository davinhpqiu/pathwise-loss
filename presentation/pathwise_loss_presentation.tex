...
\begin{frame}{From sequences to continuous paths}
  Observed time series are finite sequences of time-stamped values. Continuous-time models interpret them as samples from evolving paths and produce another path as output.

  Training therefore needs a rule for comparing whole paths. Pointwise error, elapsed-time error, derivative error, and ordered path features each express a different meaning of closeness.

  These choices can change what a model learns, particularly when observations are uneven or the relevant information lies in local ordering rather than coordinate values alone (Ramsay and Silverman, 2005; Kudrat et al., 2025).
\end{frame}
...
\begin{frame}{Path signatures}
  ...
  \]
  where $\otimes$ is the tensor product (Chen, 1957; Lyons et al., 2007). For Brownian streams, the supplied step-two rough-path lift replaces the bounded-variation assumption. A machine-learning introduction appears in Chevyrev and Kormilitzin (2016).
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.30\textwidth}
      \textbf{Level 2}\par
      Ordered pair interactions; its antisymmetric part encodes signed area
    \end{column}
    \begin{column}{0.30\textwidth}
...
\begin{frame}[plain]
  \vfill
  \centering
  {\usebeamercolor[fg]{frametitle}\huge\bfseries Experiment A: fixed path}\par
  \vfill
\end{frame}

\begin{frame}{Experiment A: fitting one path}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      \textbf{Target}\par
      A smooth two-dimensional loop contains a short burst of faster oscillation. This creates both broad shape and fine local structure in one path.
      ...
      \[
        ...
        \qquad
        \widehat Y_\theta(t)=g_\theta(z_\theta(t)).
      \]
      The experiment uses a restricted model and a more expressive model.
    \end{column}
    \begin{column}{0.47\textwidth}
      \textbf{Comparison}\par
      The model fits the same target under each loss. Target observations are either uniform in time or concentrated near the burst.

      \textbf{Purpose}\par
      This controlled setting reveals whether the loss changes the fitted path when model capacity is limited, observations are uneven, or the target contains local structure.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Experiment A: sampling density}
  ...
  Under clustered sampling, $\Jtwo$ lowers dense elapsed-time error in all six capacity-seed comparisons ($2$ capacities $\times$ $3$ seeds). Ratio-of-means reductions are \result{68.4\%} for restricted models and \result{40.8\%} for expressive models. Uniform sampling gives a 3 to 3 split.
\end{frame}
...
\begin{frame}{Experiment A: derivative supervision}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      ...
      $H^1$ training gives the best general errors in all six capacity-seed groups at update 1,000 and five of six groups at update 10,000.
...
\begin{frame}[plain]
  \vfill
  \centering
  {\usebeamercolor[fg]{frametitle}\huge\bfseries Experiment B: learned operator}\par
  \vfill
\end{frame}
...
\begin{frame}[plain]
  \vfill
  \centering
  {\usebeamercolor[fg]{frametitle}\huge\bfseries Experiment C: message sensitivity}\par
  \vfill
\end{frame}
